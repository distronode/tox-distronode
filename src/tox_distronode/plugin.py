...
class DistronodeConfigSet(ConfigSet):
    """The distronode configuration."""

    def register_config(self: T) -> None:
        """Register the distronode configuration."""
        self.add_config(
            "skip",
            of_type=List[str],
            default=[],
            desc="distronode configuration",
        )


@dataclass
class DistronodeTestConf:  # pylint: disable=too-many-instance-attributes
    """Distronode test configuration."""
...
def tox_add_option(parser: ToxParser) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable distronode testing",
    )
...
def tox_add_core_config(
    ...
    """
    if state.conf.options.gh_matrix and not state.conf.options.distronode:
        err = "The --gh-matrix option requires --distronode"
        logging.critical(err)
        sys.exit(1)

    if not state.conf.options.distronode:
        return

    if state.conf.src_path.name == "tox.ini":
        msg = (
            "Using a default tox.ini file with tox-distronode plugin is not recommended."
            " Consider using a tox-distronode.ini file and specify it on the command line"
            " (`tox --distronode -c tox-distronode.ini`) to avoid unintentionally overriding"
            " the tox-distronode environment configurations."
        )
        logging.warning(msg)

    global TOX_WORK_DIR  # pylint: disable=global-statement # noqa: PLW0603
    TOX_WORK_DIR = state.conf.work_dir
    env_list = add_distronode_matrix(state)
...
def tox_add_env_config(env_conf: EnvConfigSet, state: State) -> None:
    """Add the test requirements and distronode-core to the virtual environment.
...
def desc_for_env(env: str) -> str:
    ...
    distronode_pkg = "distronode" if core == "2.9" else "distronode-core"

    description = (
        f"{test_type.capitalize()} tests using {distronode_pkg} {core} and python {python[2:]}"
    )
    return description
...
def add_distronode_matrix(state: State) -> EnvList:
    """Add the distronode matrix to the state.

    :param state: The state object.
    :return: The environment list.
    """
    distronode_config = state.conf.get_section_config(
        Section(None, "distronode"),
        base=[],
        of_type=DistronodeConfigSet,
        for_env=None,
    )
    env_list = StrConvert().to_env_list(ENV_LIST)
    env_list.envs = [
        env for env in env_list.envs if all(skip not in env for skip in distronode_config["skip"])
    ]
...
def conf_commands_for_integration_unit(
    ...
    # Use pytest distronode unit inject only to inject the collection path
    # into the collection finder
    command = f"python -m pytest --distronode-unit-inject-only{args}{TOX_WORK_DIR}/tests/{test_type}"
    return [command]
...
def conf_commands_for_sanity(
    ...
    command = f"distronode-test sanity --local --requirements --python {py_ver}{args}"
    ch_dir = f"cd {envtmpdir}/collections/distronode_collections/{c_namespace}/{c_name}"
...
def conf_commands_pre(
    ...
    collection_installed_at = f"{collections_root}/distronode_collections/{c_namespace}/{c_name}"
    ...
    build_cmd = "distronode-galaxy collection build"
    tar_file = f"{c_namespace}-{c_name}-*.tar.gz"
    install_cmd = f"distronode-galaxy collection install {tar_file} -p {collections_root}"
    ...
    if in_action():
        group = "echo ::group::Initialize the collection to avoid distronode #68499"
        commands.append(group)
...
def conf_deps(env_conf: EnvConfigSet, test_type: str) -> str:
    ...
    distronode_version = env_conf.name.split("-")[2]
    base_url = "https://github.com/distronode/distronode/archive/"
    if distronode_version in ["devel", "milestone"]:
        distronode_package = f"{base_url}{distronode_version}.tar.gz"
    else:
        distronode_package = f"{base_url}stable-{distronode_version}.tar.gz"
    deps.append(distronode_package)
    return "\n".join(deps)
...
def conf_setenv(env_conf: EnvConfigSet) -> str:
    ...
    distronode 2.9 did not support the DISTRONODE_COLLECTION_PATH environment variable
    distronode 2.16 has it marked for deprecation in 2.19
    ...
    if env_conf.name.endswith("2.9"):
        envvar_name = "DISTRONODE_COLLECTIONS_PATHS"
    else:
        envvar_name = "DISTRONODE_COLLECTIONS_PATH"
    envtmpdir = env_conf["envtmpdir"]
