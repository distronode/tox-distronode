...
def test_distronode_environments(module_fixture_dir: Path) -> None:
    """Test that the distronode environments are available.

    :param module_fixture_dir: pytest fixture to get the fixtures directory
    """
    try:
        proc = subprocess.run(
            f"tox -l --distronode  --root {module_fixture_dir} --conf tox-distronode.ini",
            capture_output=True,
...
def test_gh_matrix(
    ...
    """Test that the distronode github matrix generation.
    ...
    proc = subprocess.run(
        f"tox --distronode --gh-matrix --root {module_fixture_dir} --conf tox-distronode.ini",
        capture_output=True,
        cwd=str(module_fixture_dir),
        text=True,
        check=True,
...
def test_environment_config(
    ...
    """Test that the distronode environment configurations are generated.
    ...
    assert "https://github.com/distronode/distronode/archive" in config["deps"]
